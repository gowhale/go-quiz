...
func (m *mainTest) Test_getCoverage_testLine_Pass() {
	tl, err := getCoverage("ok      cmd/pkg-cover      0.182s  coverage: 69% of statements")
...
func (m *mainTest) Test_getCoverage_noTestLine() {
	tl, err := getCoverage("?       go-quiz/cmd/fruit  [no test files]")
...
func (m *mainTest) Test_getCoverage_excludedFile() {
	tl, err := getCoverage("?       go-quiz  [no test files]")
...
func (m *mainTest) Test_covertOutputToCoverage_noTests_Pass() {
	commandOutput := `?       go-quiz  [no test files]
	`
...
func (m *mainTest) Test_covertOutputToCoverage_testsLine_Pass() {
	commandOutput := `ok      go-quiz/cmd/pkg-test      0.179s  coverage: 69% of statements
	`
	expectedTl := []testLine{{pkgName: "go-quiz/cmd/pkg-test", coverage: underCoverage, coverLine: true}}
...
func (m *mainTest) Test_covertOutputToCoverage_testsLine_Fail() {
	commandOutput := `ok      go-quiz/cmd/pkg-test      0.179s  coverage: wrong% of statements
	`
...
func (m *mainTest) Test_validateTestOutput_sufficentCov_testLine() {
	expectedTl := []testLine{{pkgName: "go-quiz/cmd/pkg-pizza", coverage: overCoverage, coverLine: true}}
...
func (m *mainTest) Test_validateTestOutput_NOT_sufficentCov_testLine() {
	...
	err := m.executeStruct.validateTestOutput(expectedTl, fruits.Apple)
	m.EqualError(err, "the following pkgs are not valid: [pkg=go-quiz/cmd/pkg-test cov=69.000000 under the 70.000000% minimum line coverage]")
}
