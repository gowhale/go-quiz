...
func (m *mainTest) Test_covertOutputToCoverage_testsLine_Pass() {
	commandOutput := `ok      go-quiz/cmd/pkg-test      0.179s  coverage: 79% of statements
	`
...
func (m *mainTest) Test_validateTestOutput_NOT_sufficentCov_testLine() {
	...
	err := m.executeStruct.validateTestOutput(expectedTl, fruits.Apple)
	m.EqualError(err, "the following pkgs are not valid: [pkg=go-quiz/cmd/pkg-test cov=79.000000 under the 80.000000% minimum line coverage]")
}
